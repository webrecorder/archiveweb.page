 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
class ne extends oe{}ne.directiveName="unsafeSVG",ne.resultType=2;const le=ae(ne),ce=ae(class extends re{constructor(e){var t;if(super(e),e.type!==te||"style"!==e.name||(null===(t=e.strings)||void 0===t?void 0:t.length)>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(e){return Object.keys(e).reduce(((t,i)=>{const a=e[i];return null==a?t:t+`${i=i.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${a};`}),"")}update(e,[t]){const{style:i}=e.element;if(void 0===this.ct){this.ct=new Set;for(const e in t)this.ct.add(e);return this.render(t)}this.ct.forEach((e=>{null==t[e]&&(this.ct.delete(e),e.includes("-")?i.removeProperty(e):i[e]="")}));for(const e in t){const a=t[e];null!=a&&(this.ct.add(e),e.includes("-")?i.setProperty(e,a):i[e]=a)}return T}});var de=__webpack_require__(9886),he=__webpack_require__(4841),pe=__webpack_require__.n(he);const fe="./w/api",ue=r(de.Z);function be(e){return[ue,e]}const me=window.IS_APP||window.electron&&window.electron.IS_APP||window.matchMedia("(display-mode: standalone)").matches,ge="1.8.1";function ve(e){" "==e.key&&(e.preventDefault(),e.target.click())}class we extends X{constructor(){super(),this.size="1.1em",this.width=null,this.height=null}static get properties(){return{svg:{type:String},size:{type:String},width:{type:String},height:{type:String}}}static get styles(){return o`
    :host {
  ...
  `}renderLists(){return U`
                ...
                <div class="content">
                  <div class="is-flex">
                    <div class="has-text-centered" style="width: 220px">
                      ...
                      <div style="font-size: smaller; margin-bottom: 1em">${this.getDeployType()} v${nr}</div>
                    </div>
    ...
    `}async onNewColl(e){this.showNew="loading",e.preventDefault();const t=this.renderRoot.querySelector("#new-title").value,i=JSON.stringify({metadata:{title:t}}),a=await fetch("./w/api/c/create",{method:"POST",body:i});await a.json();const r=this.renderRoot.querySelector("wr-rec-coll-index");r&&r.loadColls(),this.showNew=null}onSelectColl(e){this.selCollId=e.currentTarget.value}async setDefaultColl(){this.selCollId||(this.selCollId=await sr("defaultCollId")),!this.selCollId&&this.colls&&this.colls.length&&(this.selCollId=this.colls[0].id),self.chrome&&self.chrome.storage&&self.chrome.storage.local&&self.localStorage&&(await or("index:sortKey",localStorage.getItem("index:sortKey")),await or("index:sortDesc",localStorage.getItem("index:sortDesc")))}_setCurrColl(e){if(!(e instanceof CustomEvent))return void this.setDefaultColl();const{detail:t}=e;this.selCollId=t.coll,this.colls&&this.colls.length||(this.colls=[{id:t.coll,title:t.title}])}async onShowStart(e){this._setCurrColl(e),this.recordUrl=e.detail.url||"https://example.com/",this.showStartRecord=!0}onShowImport(e){this._setCurrColl(e),this.showImport=!0,this.isImportExisting=!0}onCollsLoaded(e){this.colls=e.detail.colls,this.setDefaultColl()}async onStartRecord(e){e.preventDefault();const t=this.renderRoot.querySelector("#url").value,i=this.renderRoot.querySelector("#preview"),a=i&&i.checked;this.showStartRecord=!1;const r=this.autorun,o=this.selCollId;if(await or("defaultCollId",o),await or("autorunBehaviors",r?"1":"0"),self.chrome&&self.chrome.runtime)chrome.runtime.sendMessage({msg:"startNew",url:t,collId:o,autorun:r});else if(window.archivewebpage&&window.archivewebpage.record){const e=!a;window.archivewebpage.record({url:t,collId:o,startRec:e,autorun:r})}return!1}async onTitle(e){if(super.onTitle(e),this.embed&&this.loadedCollId&&e.detail.replayTitle&&e.detail.title)try{await fetch(`./w/api/c/${this.loadedCollId}/pageTitle`,{method:"POST",body:JSON.stringify(e.detail)})}catch(e){console.warn(e)}}async onSaveSettings(e){e.preventDefault(),this.showSettings=!1;const t=this.renderRoot.querySelector("#ipfsDaemonUrl"),i=this.renderRoot.querySelector("#ipfsGatewayUrl");if(!t||!i)return;const a=t.value,r=i.value;return this.ipfsOpts={daemonUrl:a,useCustom:!!a,gatewayUrl:r},await this.checkIPFS(),localStorage.setItem("ipfsOpts",JSON.stringify(this.ipfsOpts)),!1}async checkIPFS(){const e=this.ipfsOpts;if(e.useCustom&&e.daemonUrl)e.message="IPFS Access -- Custom IPFS Daemon";else if(!e.daemonUrl){const t=await Ja({web3StorageToken:""});t instanceof tr&&(e.daemonUrl=t.url),e.useCustom=!1,t instanceof er?e.message="Sharing via remote web3.storage":e.daemonUrl?e.daemonUrl.startsWith("http://localhost:45")?e.message="Sharing via Brave IPFS node":e.daemonUrl.startsWith("http://localhost")?e.message="Sharing via local IPFS node":e.message="":e.message="IPFS Access Unknown - Sharing Not Available"}}}customElements.define("archive-web-page-app",cr)})();var __webpack_export_target__=self;for(var i in __webpack_exports__)__webpack_export_target__[i]=__webpack_exports__[i];__webpack_exports__.__esModule&&Object.defineProperty(__webpack_export_target__,"__esModule",{value:!0})})();